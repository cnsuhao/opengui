	...
	*/
	...
	public:
		...
		UDim operator+(const UDim& right) const  { return UDim(scale + right.scale, offset + right.offset); }
		UDim operator-(const UDim& right) const  { return UDim(scale - right.scale, offset - right.offset); }
		UDim operator*(const UDim& right) const  { return UDim(scale * right.scale, offset * right.offset); }
		UDim operator/(const UDim& right) const  { return UDim(scale / right.scale, offset / right.offset); }
		UDim operator*(const float& right) const  { return UDim(scale * right, offset); }
		UDim operator/(const float& right) const  { return UDim(scale / right, offset); }
	...
	public:
		...
		UVector2 operator+(const UVector2& right) const  { return UVector2(x + right.x, y + right.y); }
		UVector2 operator-(const UVector2& right) const  { return UVector2(x - right.x, y - right.y); }
		UVector2 operator*(const UVector2& right) const  { return UVector2(x * right.x, y * right.y); }
		UVector2 operator/(const UVector2& right) const  { return UVector2(x / right.x, y / right.y); }
		UVector2 operator*(const FVector2& right) const  { return UVector2(x / right.x, y / right.y); }
		UVector2 operator/(const FVector2& right) const  { return UVector2(x / right.x, y / right.y); }

		bool operator==(const UVector2& right) const  { return x==right.x && y == right.y; }
		bool operator!=(const UVector2& right) const  { return !operator==(right); }
	...
	public:
		...
		const UVector2& getPosition() const { return min; }
		UVector2 getSize() const { return max - min; }
		UDim getWidth() const { return max.x - min.x; }
		UDim getHeight() const { return max.y - min.y; }

		void setWidth(const UDim& newWidth) { max.x = min.x + newWidth; }
		void setHeight(const UDim& newHeight) { max.y = min.y + newHeight; }
		void setSize(const UVector2& newSize) { max = min + newSize; }
